...
def challenge3():
    ...
    for g in saved:
        try:
            ...
            #sevenChar = "equality"
            print(urllib3.request.urlopen("http://www.pythonchallenge.com/pc/def/{}.html".format(sevenChar)))
        except IOError:
            print("404 Error")

    #for x in range(0,10):
    #    print(str(saved[x])[-11:-2])
    #    print(urllib3.request.urlopen("http://www.pythonchallenge.com/pc/def/{}.html".format(str(saved[x])[-11:-2])))
...
def challenge4():
    ...
    while EOF == False:
        ...
        #pyurl = urllib3.request.Request(url='http://www.pythonchallenge.com/pc/def/linkedlist.php?nothing=' + str(pc4))
...
def challenge5():
    ...
    with open("pychallenge5.pkl","rb") as pickle_file:
        try:
...
def challenge10():
    ...
    for x in range(0,len(calculated_string_array)):
        if calculated_string_array[0][0] == calculated_string_array[z][x]:
            #ERROR
            print(f"same - {calculated_string_array[0][0]} - {calculated_string_array[0][x]}")
...
def test():
    ...
    while loop in range(0,31):
        if new_string != "":
            eval_string = new_string
        array_pointer = 0
        count = 0
        for x in eval_string:
            if array_pointer == 0:
                set_char = x
                count = 1
                array_pointer = 1
                ...
                count += 1
                array_pointer += 1
                if len(eval_string) == array_pointer:
                    new_string = new_string + str(count) + set_char
            elif x != set_char:
                new_string = new_string + str(count) + set_char
                set_char = x
                count = 1
        ...
        print(new_string)
        print(len(new_string)) #9898
        list_of_values.append(new_string)
        print(f"loop= {loop}")
        loop += 1
...
# view-source:http://www.pythonchallenge.com/pc/return/5808.html
#####################################################################
